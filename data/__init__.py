...
def define_dataloader(logger, opt):
    ...
    phase_dataset, val_dataset, debug_dataset = define_dataset(logger, opt)
    ...
    # debug
    if opt['debug1']:
        dataloader_args = opt['datasets']['debug']['dataloader']['args']
        debug_dataloader = DataLoader(debug_dataset, sampler=data_sampler, worker_init_fn=worker_init_fn, **dataloader_args)
    else:
    ...
    if opt['global_rank']==0 and val_dataset is not None:
        ...
        val_dataloader = None
    return dataloader, val_dataloader, debug_dataloader
...
def define_dataset(logger, opt):
    ...
    if opt['mask_path'] != None:
        dataset_opt['args']['mask_config']['mask_path'] = opt['mask_path']
    phase_dataset = init_obj(dataset_opt, logger, default_file_name='data.dataset', init_type='Dataset')
    val_dataset = None

    # debug
    if opt['debug1']:
        debug_opt = opt["datasets"]['debug']['which_dataset']
        debug_dataset = init_obj(debug_opt, logger, default_file_name='data.dataset', init_type='Dataset')
    else:
        debug_dataset = None
    # debug_dataloader_opt = opt["datasets"]['debug']['dataloader']
    ...
    return phase_dataset, val_dataset, debug_dataset
...
def subset_split(dataset, lengths, generator):
    ...
    for offset, length in zip(np.add.accumulate(lengths), lengths):
        if length == 0:
            ...
            # class Subset comes from dataset.py
