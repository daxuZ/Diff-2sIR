...
class BaseModel():
    def __init__(self, opt, phase_loader, val_loader, debug_loader, metrics, logger, writer):
        ...
        #----------------------------------------
        self.results_dict_v = CustomResult([],[])
    ...
    def load_network_v(self, network, network_label, strict=True):
        if self.opt['path']['resume_state'] is None:
            return
        self.logger.info('Beign loading pretrained v_model [{:s}] ...'.format(network_label))
        model_path = "{}_{}.pth".format(self.opt['path']['resume_state_v'], network_label)
        if not os.path.exists(model_path):
            self.logger.warning('Pretrained model in [{:s}] is not existed, Skip it'.format(model_path))
            return
    ...
    # def resume_training(self):
    #     """ Resume the optimizers and schedulers for training, only works when phase is train or resume training is enabled """
    #     if self.phase != 'train' or self.opt['path']['resume_state'] is None:
    #         return
    ...
    #     state_path = "{}.state".format(self.opt['path']['resume_state'])
    #     if not os.path.exists(state_path):
    #         self.logger.warning('Training state in [{}] does not exist, skipping it'.format(state_path))
    #         return
    #     self.logger.info('Loading training state from [{}] ...'.format(state_path))
    #     resume_state = torch.load(state_path, map_location=lambda storage, loc: self.set_device(storage))
    #     resume_optimizers = resume_state.get('optimizers', [])
    #     resume_schedulers = resume_state.get('schedulers', [])
    #
    #     # Debugging information
    #     self.logger.debug('Loaded optimizers: {}'.format(resume_optimizers))
    #     self.logger.debug('Loaded schedulers: {}'.format(resume_schedulers))
    ...
    #     for i, o in enumerate(resume_optimizers):
    #         try:
    #             # Convert saved_groups to match current groups
    #             saved_groups = deepcopy(o['param_groups'])
    #             for group, saved_group in zip(self.optimizers[i].param_groups, saved_groups):
    #                 saved_group['params'] = [p for p in group['params']]
    #             o['param_groups'] = saved_groups
    #             self.optimizers[i].load_state_dict(o)
    #         except Exception as e:
    #             self.logger.error('Error loading optimizer state at index {}: {}'.format(i, e), exc_info=True)
    #             raise e
    #     for i, s in enumerate(resume_schedulers):
    #         try:
    #             self.schedulers[i].load_state_dict(s)
    #         except Exception as e:
    #             self.logger.error('Error loading scheduler state at index {}: {}'.format(i, e), exc_info=True)
    #             raise e
    #     self.epoch = resume_state['epoch']
    #     self.iter = resume_state['iter']
